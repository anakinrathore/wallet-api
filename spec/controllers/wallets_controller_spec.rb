...
RSpec.describe WalletsController, type: :controller do
  ...
  describe 'POST#deposit' do
    it 'should increase the wallet balance when amount is positive' do
      ...
      post :deposit, params: {  "user_id": user.id, "wallet_id": wallet.id, "deposit_amount": 40000 }
      wallet.reload
    ...
    it 'should not increase the wallet balance when amount is negative' do
      user = create(:user)
      wallet = user.wallets.create(balance_in_paise:500)
      post :deposit, params: {  "user_id": user.id, "wallet_id": wallet.id, "deposit_amount": -600 }
      wallet.reload
  ...
  describe 'POST#withdraw' do
    it 'should decrease the wallet balance when amount is positive' do
      ...
      post :withdraw, params: {  "user_id": user.id, "wallet_id": wallet.id, "withdrawl_amount": 40000 }
      wallet.reload
    ...
    it 'should not decrease the wallet balance when amount is negative' do
      user = create(:user)
      wallet = user.wallets.create(balance_in_paise:500)
      post :withdraw, params: {  "user_id": user.id, "wallet_id": wallet.id, "withdrawl_amount": -600 }
      wallet.reload
      expect(wallet.balance_in_paise).to eq 500
      expect(response).to have_http_status 200
    end
    ...
    it 'should not decrease the wallet balance when amount is 0' do
      ...
      post :withdraw, params: {  "user_id": user.id, "wallet_id": wallet.id, "withdrawl_amount": 500 }
      wallet.reload
